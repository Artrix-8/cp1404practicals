...
def is_long_word(word, length=5):
    """
    Determine if the word is as long or longer (i.e. >= ) than the length passed in
    >>> is_long_word("not")
    False
...
def run_tests():
    ...
    assert test_car.fuel == 10, "Car does not pass values into fuel correctly"
...
def phrase_to_sentence(phrase):
    """
    Reformat a phrase to begin with a capital and end with a period (if needed).
    >>> phrase_to_sentence('hello')
    'Hello.'
    >>> phrase_to_sentence('It is an ex parrot.')
    'It is an ex parrot.'
    >>> phrase_to_sentence('aig  iawi fjnarjhbh.')
    'Aig  iawi fjnarjhbh.'
    """
